# Contributing to crito

Thanks for helping out. `crito` is a lean, zero-infra, BYOK GitHub Action
...
The agent lives in the `crito/` package. Each module has one job:

| Module | Role |
| --- | --- |
| `review.py` | Orchestrator / entrypoint (`python -m crito.review`): wires config → diff → secrets → prompt → ensemble → postprocess → GitHub. |
| `config.py` | Loads `.crito.yaml` + `.crito/rules.md`, applies env overrides, returns a `Config`. Precedence: dataclass defaults < yaml < env. |
...
`crito.openrouter`, and `crito.github_client` were **not** imported.
...
asserts that `crito.openrouter`, `crito.github_client`, and `httpx` were
...
Adding a `.crito.yaml` key is a five-step change. A key that is *read but
...
- **Run both checks** (`python tests/test_smoke.py` and
  `python -m py_compile crito/*.py`) before pushing.
- **Open a PR against `main`.** The crito reviews its own PRs — expect
