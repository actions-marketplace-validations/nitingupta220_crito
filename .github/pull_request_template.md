<!--
Thanks for contributing to review-agent! Keep changes lean and in-lane:
...
- [ ] `python -m py_compile prreview/*.py` is clean
...
- [ ] Any new `.pr-review.yaml` config key is **wired** in `config.load_config` **and** documented in the README (don't document keys `load_config` doesn't read)
