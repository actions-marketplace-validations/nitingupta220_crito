...
1. **Quota math (load-bearing).** Does a `models[]` fallback **share** or **multiply** the per-account daily free quota? Failed attempts count against the cap. If fallback debits the same counter, a 5-model array can burn up to 5× quota on one struggling PR — the opposite of "multiplying your ceiling." **Verify via `GET /api/v1/key` under a forced fallback before treating the array as the default path.**
