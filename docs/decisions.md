...
1. **Quota math (load-bearing).** ~~Does a `models[]` fallback share or multiply the daily free quota?~~ **Largely settled by the 2026-06-02 spike + exhaustion run** ([`spike-openrouter-quota.md`](spike-openrouter-quota.md)): the array is hard-capped at **3**, and a ~80-request / ~180-sub-attempt exhaustion run **never hit an account daily limit** — the cited "50/day" cap did not reproduce, and the fan-out-burns-quota fear did not materialize. The real constraint is **upstream provider saturation**, which the fallback array mitigates (97% success live, lead model was 0/60). **Decision: build the ≤3 fallback array as default.** Residual: confirm current official limits before launch (modest sample; weekly token-allocation ceiling untested); keep one-call-per-PR discipline + liveness-aware lead selection (to avoid ~7s fallthrough latency).
